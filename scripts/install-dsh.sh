#!/usr/bin/env bash
# ============================================================================
# install-dsh.sh — 把 dsh-learning-starter 装入 DSH（DeepSeek Harness）
#
# 装入内容：
...
#   - 安装后新开一个 DSH 会话（选择"学习模式"preset）即可生效；
#     正在运行的会话保持旧 persona，不需要也不应该重启 3082。
# ============================================================================
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PRESET_SRC="$ROOT/presets/learning"
...
echo "== dsh-learning-starter 安装 (mode=$MODE) =="
echo "  DSH_HOME = $DSH_HOME"

if [ "$MODE" = "uninstall" ]; then
    PRESET_DST="$DSH_HOME/.agent-presets/learning"
    if [ -e "$PRESET_DST" ]; then
        mv "$PRESET_DST" "${PRESET_DST}.bak-$TS"
        echo "  移除 preset: learning → .bak-$TS"
    else
        echo "  preset 不存在，跳过"
    fi
...
echo "== 1/1 agent preset（含 skills/）→ $DSH_HOME/.agent-presets/learning =="
PRESET_DST="$DSH_HOME/.agent-presets/learning"
if [ -e "$PRESET_DST" ]; then
    mv "$PRESET_DST" "$PRESET_DST.bak-$TS"
    echo "  备份: learning → learning.bak-$TS"
fi
mkdir -p "$PRESET_DST"
